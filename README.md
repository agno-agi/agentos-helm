...
5. **AgentOS UI.** Chat with agents, inspect sessions, traces, memory, and evals from the control plane at [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-helm&utm_content=agentos-helm&utm_term=kubernetes).
...
Production is JWT-gated, so this connection needs a token — `uvx agno connect` mints a service-account token (`agno_pat_…`) for it. `--url` pins the target; a bare `uvx agno connect` resolves it from where you run it — it reads `AGENTOS_URL` from `.env.production`/`.env` in the cwd, and falls back to `http://localhost:8000/mcp`.

For **claude.ai and ChatGPT (web)**, the platform serves its own OAuth authorization server on `/mcp` — `up.sh` generated the login secret for it (`MCP_CONNECT_SECRET` in `.env.production`) during deploy, provided the deploy had a public URL (deployed without `INGRESS_HOST`? set `MCP_CONNECT_SECRET` and a public `AGENTOS_URL` in `.env.production` and run `./scripts/k8s/env-sync.sh`). Add `https://<your-agentos-domain>/mcp` as a custom connector in the chat app's connector settings. **Leave the form's optional OAuth fields (client ID / client secret) empty** — the platform registers the app automatically. The connect secret is typed exactly once, on the consent page that opens after you click **Connect**.
