...
For **claude.ai and ChatGPT (web)**, follow the remote MCP server registration process for `https://<your-agentos-domain>/mcp` and authenticate using OAuth.
...
| `AGENTOS_URL` | no | `http://127.0.0.1:8000` | Scheduler base URL. The chart resolves it automatically (explicit value > ingress URL > in-cluster service DNS); set by hand only for a custom domain or tunnel. |
