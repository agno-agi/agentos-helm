...
6. Ask me which frontends I want connected, then set up the ones I pick:
   ...
   - Claude and ChatGPT apps (web or desktop): their sessions run in the cloud and can't reach localhost, so work with me on deploying to production first using ./scripts/k8s/up.sh with INGRESS_HOST set (needs kubectl pointed at a cluster, helm, and an image the cluster can pull; the script pauses while I mint a JWT key at os.agno.com, and saves the MCP_CONNECT_SECRET — the OAuth consent secret — in .env.production). Then I add https://<my-agentos-domain>/mcp as a custom connector in the chat app's connector settings and approve the consent page with that secret.
